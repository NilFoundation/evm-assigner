...
{
    state.m_handler->set_witness(0, 0, 0, stack[0]);
    state.m_handler->set_witness(0, 1, 0, stack[1]);
    stack.top() *= stack.pop();
}
...
inline void callvalue(StackTop stack, ExecutionState& state) noexcept
{
    ...
    state.m_handler->set_witness(1, 0, 0, val);
    stack.push(val);
}
...
inline void calldataload(StackTop stack, ExecutionState& state) noexcept
{
    ...
    state.m_handler->set_witness(1, 0, 2, index);
...
inline Result mload(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    state.m_handler->set_witness(2, 0, 2, index);
    return {EVMC_SUCCESS, gas_left};
}
...
inline Result mstore(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    state.m_handler->set_witness(2, 0, 0, value);
    state.m_handler->set_witness(2, 0, 1, index);
...
inline void tload(StackTop stack, ExecutionState& state) noexcept
{
    ...
    state.m_handler->set_witness(3, 1, 2, x);
}
...
inline Result tstore(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    state.m_handler->set_witness(3, 1, 0, stack[1]);
    state.m_handler->set_witness(3, 1, 1, stack[0]);
...
{
    auto& index = stack.top();
    state.m_handler->set_witness(1, 0, 1, index);
