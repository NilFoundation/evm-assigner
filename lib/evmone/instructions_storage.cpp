...
Result sload(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    state.m_handler->set_witness(static_cast<std::uint32_t>(state.msg->depth), 3, 2, x);
...
Result sstore(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    state.m_handler->set_witness(static_cast<std::uint32_t>(state.msg->depth), 3, 0, stack[1]);
    state.m_handler->set_witness(static_cast<std::uint32_t>(state.msg->depth), 3, 1, stack[0]);
